 ...
 * \file regrtest1.cpp
 *
 * \brief   regression test for run-time Quantity library.
...
void check_powers_and_roots()
{
    ...
    REPORT( "nth_power<10>", nth_power( f1, 10 ), "1024.000000 m+10 kg+10 s-20" );
    REPORT( "nth_power<4>", nth_power( f1, 4 ), "16.000000 m+4 kg+4 s-8" );
    REPORT( "nth_power<3>", nth_power( f1, 3 ), "8.000000 m+3 kg+3 s-6" );
    REPORT( "nth_power<2>", nth_power( f1, 2 ), "4.000000 m+2 kg+2 s-4" );
    REPORT( "nth_power<1>", nth_power( f1, 1 ), "2.000000 m kg s-2" );
    REPORT( "nth_power<0>", nth_power( f1, 0 ), "1.000000" );
    REPORT( "nth_power<-1>", nth_power( f1, -1 ), "0.500000 m-1 kg-1 s+2" );
    REPORT( "nth_power<-2>", nth_power( f1, -2 ), "0.250000 m-2 kg-2 s+4" );
    ...
    REPORT( "nth_root<4>", nth_root( hv, 4 ), "10.000000 m" );
    REPORT( "nth_root<2>", nth_root( hv, 2 ), "100.000000 m+2" );
    REPORT( "nth_root<1>", nth_root( hv, 1 ), "10000.000000 m+4" );
    REPORT( "nth_root<-1>", nth_root( hv, -1 ), "0.000100 m-4" );
    REPORT( "nth_root<-2>", nth_root( hv, -2 ), "0.010000 m-2" );
    REPORT( "nth_root<-4>", nth_root( hv, -4 ), "0.100000 m-1" );
...
void check_user_defined()
{
    ...
    quantity full_power = 3270000L * hyperspace::scotty();
    ostringstream os;
...
#if 0
inline quantity meter_f()
{
   return quantity( length_d, 1.0f );
}

inline quantity meter_d()
{
   return quantity( length_d, 1.0 );
}

inline quantity meter_ld()
{
   return quantity( length_d, 1.0L );
}

void check_type_promotion()
{
  {
#ifdef PHYS_UNITS_COMPILER_IS_GNUC
   std::string result   ( "d" );
   std::string result_ld( "e" );
#else
   std::string result   ( "double" );
   std::string result_ld( "long double" );
#endif

    REPORT( "promote(float,double)", typeid( detail::promote<float,double>::type ).name(), result );
    REPORT( "promote(double,float)", typeid( detail::promote<double,float>::type ).name(), result );

    REPORT( "promote(float,long double)", typeid( detail::promote<float,long double>::type ).name(), result_ld );
    REPORT( "promote(long double,float)", typeid( detail::promote<long double,float>::type ).name(), result_ld );
  }

#if defined( PHYS_UNITS_COMPILER_IS_MSVC2010 )
  {
    auto x = 1.0L * meter_f();
    auto y = meter_f() * 1.0L;
    std::string result( "class phys::units::quantity<struct phys::units::dimensions<1,0,0,0,0,0,0>,long double>" );
    REPORT( "1.0L * q-float", typeid( x ).name(), result );
    REPORT( "q-float * 1.0L", typeid( y ).name(), result );
  }
#endif

    REPORT( "q-float + q-double", (meter_f() + meter_d()), "2.000000 m" );
    REPORT( "q-double + q-float", (meter_d() + meter_f()), "2.000000 m" );

    REPORT( "q-float - q-double", (meter_f() - meter_d()), "0.000000 m" );
    REPORT( "q-double - q-float", (meter_d() - meter_f()), "0.000000 m" );

    REPORT( "q-float * double", (meter_f() * 1.0), "1.000000 m" );
    REPORT( "double * q-float", (1.0 * meter_f()), "1.000000 m" );

    REPORT( "q-float * q-double", (meter_f() * meter_d()), "1.000000 m+2" );
    REPORT( "q-double * q-float", (meter_d() * meter_f()), "1.000000 m+2" );

    REPORT( "q-float / double", (meter_f() / 1.0), "1.000000 m" );
    REPORT( "double / q-float", (1.0 / meter_f()), "1.000000 m-1" );

    REPORT( "q-float / q-double", (meter_f() / meter_d()), "1.000000" );
    REPORT( "q-double / q-float", (meter_d() / meter_f()), "1.000000" );
}
#endif
